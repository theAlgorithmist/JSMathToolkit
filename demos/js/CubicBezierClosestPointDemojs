                  ...
                  )
{
  ...
  function onPageLoaded()
  {
    ...
    boundingShape           = new createjs.Shape();
...
//    p0Marker.set_y(yAxis, axisHeight, -3);
    ...
    p0Marker.set_x(xAxis, axisLength, -6);
    p0Marker.set_y(yAxis, axisHeight, 0.5);
...
//    curve.set_y0(-3);
  ...
  function drawCurve()
  {
    ...
    var tminX = bezierUtils.tAtMinX(curve);
    var tmaxX = bezierUtils.tAtMaxX(curve);
    var tminY = bezierUtils.tAtMinY(curve);
    var tmaxY = bezierUtils.tAtMaxY(curve);
    
    // convert coordinate space to pixel offsets in the Canvas
    var minX = (curve.getX(tminX) - defaultLeft)*pxPerUnitX;
    var maxX = (curve.getX(tmaxX) - defaultLeft)*pxPerUnitX;
    var minY = (defaultTop - curve.getY(tminY))*pxPerUnitY;
    var maxY = (defaultTop - curve.getY(tmaxY))*pxPerUnitY;
    
    var g = boundingShape.graphics;
    g.clear();
