/*
 * Supporting script for 2D Graph Slice Tool - this renders non-interactive graph axes.  Copyright (c) 2013, Jim Armstrong (The Algorithmist).  All Rights Reserved. 
 */
require(['jquery', 'bootstrap', 'easeljs', '../../core/GraphSlice', '../../graphing/GraphAxis'], function($, bootstrapRef, easelJS, GraphSliceModule, GraphAxisModule)
{
  var stage;
  var xAxis;
  var yAxis;
  ...
  $('#reset').click(function()
  {
    ...
    $('#instruct').text("Drag the triangular marker to slice the graph and observe the new coordinate values.");
  });
  ...
  {
    if( sliceTool.getDirection() == "horizontal" )
    {
      $('#instruct').text( "y = " + value.toFixed(2) );
    }
    else
    {
      $('#instruct').text( "x = " + value.toFixed(2) );
    }
  };
  
  function drawAxes()
  {
    // the graphic context is passed into the axis - only clearing, line properties, and fills are directly managed
    var g = grid.graphics;
    g.clear();
    g.setStrokeStyle(1);
