/*
 * 2D Convex Hull Demo.  Copyright (c) 2014, Jim Armstrong (The Algorithmist).  All Rights Reserved. 
 */
require(['jquery', 
         'bootstrap', 
         'easeljs', 
                  ...
                  )
{
  ...
  var convexHullRef = new ConvexHullModule();
  var convexHull    = new convexHullRef.ConvexHull();
  
  var points  = [];
  
  // simple polyline - use this for melkman's algorithm
//  var _x = [-5, -2, 1, 1, 0, 4,  0, -1,  4];
//  var _y = [ 3,  2, 3, 2, 0, 0, -4, -1, -2];
  
  // similar but arbitrary point set - use either the previous set or this one for graham scan
  var _x = [4, -5, -2, 1, 1, 4, 0,  0, -1, -7];
  var _y = [-2, 3,  2, 3, 2, 0, 0, -4, -1, 0];
  ...
  for( i=0; i<len; ++i)
  {
    points.push( new pointRef.Point(_x[i], _y[i]) );
  }
  ...
  function onPageLoaded()
  {
    ...
    hullRenderShape  = new createjs.Shape();
    ...
    stage.addChild(hullRenderShape);
  ...
  function drawHull()
  {
    ...
    // graham scan - works on any point set, but more memory intense
    var hull = convexHull.grahamScan(points);
    
    // melkman's algorithm - inputs must be simple polyline
    // var hull = convexHull.melkman(points);
    
    var p  = hull[0];
    var xh = (p.get_x() - defaultLeft)*pxPerUnitX;
    var yh = (defaultTop - p.get_y())*pxPerUnitY;
    len    = hull.length;
    ...
    {
      p  = hull[i];
      xh = (p.get_x() - defaultLeft)*pxPerUnitX;
      yh = (defaultTop - p.get_y())*pxPerUnitY;
    ...
    $('#instruct').text( "# points in convex hull: " + len );
  };
